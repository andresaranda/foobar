...
def solution(x, y):
    """
    This function finds the worker ID of a bunny at location (x,y).
    It requires 2 coordinates as integers or floats and
    returns the ID as a string

    Note: sum of a series of integers from 1 to n: n*(n+1)/2
    Unsimplified equation: ID = x*(x+1)/2 + ( (x+y-2)*(x+y-1)/2 - (x-1)*x/2 )
    """

    ID = int(x + (x + y - 2) * (x + y - 1) / 2)
    return str(ID)

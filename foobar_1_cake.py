...
def solution(string):
    """
    This function finds the maximum number of equal parts
    that a string can be divided in. It requires a non-empty
    ...
    for size in range(1, length + 1):
        if (length % size) == 0:
            amount = int(length / size)
            prev_substring = ''
            for i in range(0, amount):
                substring = string[(i * size): ((i + 1) * size)]
...
num = solution("fryfujg jb vhfryfujg jb vh")
